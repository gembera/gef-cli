 ...
 * @returns {String} Converted value
 */
function resolveEnvVars(varValue, envVars) {
  ...
  return varValue.replace(
    ...
    (_, escapeChars, varNameWithDollarSign, varName, altVarName) => {
      ...
      return (
        ...
        ((envVars || process.env)[varName || altVarName] || '')
      )
    },
  )
}
 ...
 * @returns {String} Converted value
 */
function varValueConvert(originalValue, originalName, envVars) {
  return resolveEnvVars(replaceListDelimiters(originalValue, originalName), envVars)
}
