...
function crossEnv(args, options = {}) {
  ...
  if (command) {
    ...
    let $command = commandConvert(command, env, true)
    // by default normalize is `false`, so not run for cmd args
    let $args = commandArgs.map(arg => commandConvert(arg, env))
...
function getEnvVars(envSetters) {
  ...
  Object.keys(envSetters).forEach(varName => {
    envVars[varName] = varValueConvert(envSetters[varName], varName, envVars)
  })
  return envVars
}
